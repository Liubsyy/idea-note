...
在引用段落中按回车时，Idea Note 会自动插入空白引用行，使新内容成为真正的新段落，而不是被 Markdown 合并的软换行；实时预览会将空白引用行压缩为紧凑的段落间距。在这条空白引用行的 `>` 后按回退键，会删除整条分隔行并恢复为单换行，不会残留空行。

### 高亮块（Idea Note 扩展）

高亮块使用特殊引用标记，块内仍可使用加粗、列表、链接等 Markdown 语法：

> [!HIGHLIGHT color=blue]
>
> 这是一个支持 **Markdown** 的高亮块。
>
>  - 可以包含列表
> - 也可以包含[链接](https://example.com)

颜色写在标记中，可选 `yellow`、`blue`、`green`、`red`、`purple`；省略颜色时默认为蓝色：

> [!HIGHLIGHT color=blue]
>
> 这是一个蓝色高亮块。

工具栏还提供扩展色板和自定义取色。自定义颜色保存为六位十六进制色值：

> [!HIGHLIGHT color=#8b5cf6]
>
> 这是一个自定义颜色的高亮块。

标记行和正文之间保留一条空白引用行 `>`，使两者成为独立的 Markdown 段落。在普通 Markdown 编辑器中，它会退化为包含 `[!HIGHLIGHT]` 标记的引用块：虽然没有彩色样式，但标记与正文不会合并成一行，也不会丢失正文。手写块内空行时要保留一行 `>`，否则 Markdown 会把引用块结束在空行处。

高亮块的换行规则与普通引用块一致：按回车时自动插入空白引用行 `>`，开始一个新段落；实时预览会将这条空白引用行压缩为紧凑的段落间距。

高亮块在实时预览和 PDF 导出中与正文左侧对齐，不显示普通引用块的左侧边框；普通引用块的缩进和边框样式不受影响。

AI 笔记助手也会收到上述语法规范。让它新建或编辑高亮块时，会使用带空白引用行的新格式，并在修改正文时保留现有颜色和引用结构。
...
指定显示尺寸时写成 `<img>` 标签——标准 Markdown 没有尺寸语法，`<img>` 是各家渲染器（含 GitHub）都认的唯一写法。数字为像素，也可用百分比；只写一边时另一边按比例缩放：

<img src="./assets/markdown-sample.svg" width="200">

<img src="./assets/markdown-sample.svg" alt="指定宽高" width="200" height="120">

<img src="./assets/markdown-sample.svg" width="50%">

不设尺寸时保持默认写法 `![](<./assets/markdown-sample.svg>)`，图片按原始尺寸显示。工具栏的图片按钮在插入时即可填写宽 / 高；光标位于图片上（`![](…)` 或 `<img>` 都算）时该按钮会高亮，点击可就地修改这张图片的地址、替代文本与尺寸——填了尺寸就写成 `<img>`，清空尺寸又会变回 `![](…)`（手写的 `class`、`style` 等属性会保留，此时保持 `<img>` 不变）。

Typora 风格的 `![](img.png =200x120)` 仍可正常读取与渲染（兼容既有笔记），但本应用不再写出这种格式：它不是合法 CommonMark，严格渲染器会把整行当普通文字显示。
...
|------|:--------:|------|
| ATX 标题 | 是 | H1-H6 |
| 高亮块 | 是 / Idea Note 扩展 | 支持预设、扩展色板和自定义颜色，其他编辑器退化为引用块 |
| Setext 标题 | 兼容性测试 | 常见变体 |
| 列表嵌套 | 是 | 多级 |
| 代码块 | 是 | 围栏、缩进、行号 |
| 表格 | 是 | GFM，对齐与基础单元格 |
| 任务列表 | 是 | GFM，复选框可切换 |
| 图片 | 是/兼容性测试 | 网络、相对路径、本地 GIF；`<img>` 标签同样支持本地路径，并用 `width`/`height` 指定显示尺寸；`=宽x高` 仅兼容读取；引用式为兼容性测试 |
