...
Prompt 里还写着两条 Idea Note 特有的 Markdown 扩展，让模型写出来的内容能被编辑器正确渲染：

- **高亮块**：`> [!HIGHLIGHT color=blue]` 起头的引用块扩展，Prompt 里给出了三行起始结构、颜色取值和"每行都要保留 `>` 前缀"这些硬性要求。
- **可交互组件**：告诉模型笔记里可以嵌入「`input` 参数块 + 可运行代码块」，用户要计算器、能调参数的图表、自动重算的数据表时应当用它实现；同时要求模型**动手前先调用 `component_guide` 查规范**，不要凭记忆猜语法，并且只写计算和渲染逻辑（代码会在用户本机真实执行）。

```
Idea Note 的笔记里还可以嵌入「可交互组件」……
写或修改可交互组件之前，先调用 component_guide 查阅规范（默认 topic=overview；
需要细节时再取 input / data / output / run / examples），不要凭记忆猜语法……
组件代码是在用户电脑上真实执行的：只写计算和渲染逻辑，不要写删除文件、
联网上传、安装软件之类的脚本。
```

高亮块的语法很短，直接写进 Prompt 就够了；可交互组件的完整规范有上万字，塞进每次请求既浪费上下文又拖慢响应，所以拆成了 `component_guide` 工具，用到才取（见 3.2）。
...
| `component_guide` | 查阅可交互组件的写法规范，并获知本机启用了哪些运行器。要在笔记里做计算器、图表、可调参数的小工具时先调用它 | topic（可选） |

`read_open_file`、`edit_open_file`、`write_open_file` 针对"当前打开的这一篇"；`read_file`、`create_note`、`delete_file`、`search_notes` 针对"整个笔记工作区"；`component_guide` 谁也不碰，只是把写组件要用的规范交给模型。这样自然语言就能覆盖日常笔记操作：
...
- "在这篇笔记里加一个房贷计算器" → component_guide → edit_open_file
...
### 3.2 `component_guide`：把长规范做成工具

前七个工具都在改变或读取用户的文件，`component_guide` 是唯一一个**只返回文档**的工具：它不碰磁盘、没有副作用，因此不需要任何确认，在"每步确认"模式下也直接执行，卡片上只显示"查阅 可交互组件规范（overview）"。

这么设计有三个原因：

- **省上下文**：完整规范上万字，按 `overview` / `input` / `data` / `output` / `run` / `examples` 六个主题切开，模型只取当前用得上的那一块。
- **少出错**：围栏属性（`in=` / `out=` / `run=` / `result=`）、参数块 DSL 和"stdout 最后一行必须是单行 JSON"的输出协议都很容易记岔，让模型现查一遍比事后返工便宜。
- **贴合本机**：返回内容末尾会附上当前设备**已启用的运行器**、代码块执行总开关和是否开启运行前确认，模型因此不会写出一段用户根本跑不起来的语言。
