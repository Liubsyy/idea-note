...
| 系统 | 文件 | 说明 |
| :--- | :--- | :--- |
| **Windows** | **x64**：[安装包](https://github.com/Liubsyy/idea-note/releases/latest/download/Idea.Note_1.2.2_windows_x64_setup.exe) \| [免安装包](https://github.com/Liubsyy/idea-note/releases/latest/download/Idea.Note_1.2.2_windows_x64.zip)<br>**x86**：[安装包](https://github.com/Liubsyy/idea-note/releases/latest/download/Idea.Note_1.2.2_windows_x86_setup.exe) \| [免安装包](https://github.com/Liubsyy/idea-note/releases/latest/download/Idea.Note_1.2.2_windows_x86.zip) | 大多数电脑选 x64<br>32 位系统选 x86 |
| **MacOS** | **Apple Silicon**：[安装包](https://github.com/Liubsyy/idea-note/releases/latest/download/Idea.Note_1.2.2_macos_aarch64.dmg) \| [应用包压缩](https://github.com/Liubsyy/idea-note/releases/latest/download/Idea.Note_1.2.2_macos_aarch64.app.tar.gz)<br>**Intel**：[安装包](https://github.com/Liubsyy/idea-note/releases/latest/download/Idea.Note_1.2.2_macos_x64.dmg) \| [应用包压缩](https://github.com/Liubsyy/idea-note/releases/latest/download/Idea.Note_1.2.2_macos_x64.app.tar.gz) | M芯片选 Apple Silicon<br>Intel 芯片选 Intel  |
| **Linux** | **安装包**：[deb](https://github.com/Liubsyy/idea-note/releases/latest/download/Idea.Note_1.2.2_linux_amd64.deb) \| [rpm](https://github.com/Liubsyy/idea-note/releases/latest/download/Idea.Note_1.2.2_linux_x86_64.rpm)<br>**免安装**：[AppImage](https://github.com/Liubsyy/idea-note/releases/latest/download/Idea.Note_1.2.2_linux_amd64.AppImage) | Ubuntu/Debian/Linux Mint选deb<br>Fedora/RHEL/CentOS Stream/openSUSE选rpm |
...
- **全屏演示**：全屏连续滚动查看当前文件，也可直接按 `F5` 进入。
- **分页演示**：按屏幕可用高度自动分页，正文中的 Markdown 分隔线 `---` 可以提前换页。分隔线前后留出空行；代码块和表格中的 `---` 不会触发分页。窗口大小、字号或图片尺寸变化时会重新计算分页。

分页演示可点击上一页 / 下一页，或使用方向键、`PageUp` / `PageDown`、空格翻页；`Home` / `End` 跳到首页 / 末页。两种模式都支持缩放，按 `Esc` 退出并返回原笔记。
...
运行代码块上面可以加上一个参数控件块，笔记就变成了一个可交互的小工具。读者拖滑块、改数字、选日期，脚本就在本机重跑一遍，结果直接渲染回笔记里——不用打开终端，也不用改代码。
...
口令本身不会保存到任何地方。首次设置时会生成一串仅显示一次的恢复码，请将它存入密码管理器或妥善抄录；如果口令和恢复码同时丢失，加密内容将无法恢复。设置中还支持修改口令、重新生成恢复码和重置主密钥。
...
- **代码块执行**：可运行的代码块语言、解释器命令与超时、输出上限
- **AI 笔记助手**：模型服务、API Key、字号与会话历史保留数量
- **远程同步**：Git 仓库与同步代理
